...
void writeMotors() { // [1000;2000] => [125;250]
  /****************  Specific PWM Timers & Registers for the MEGA's   *******************/
        ...
        OCR3C = motor[0]<<3; //  pin 3, rear right/ link3
      #else
    ...
    #if (NUMBER_MOTOR > 1)
      #ifndef EXT_MOTOR_RANGE 
        OCR3A = motor[1]<<3; //  pin 5, front right/ link1
      #else
    ...
    #if (NUMBER_MOTOR > 2)
      #ifndef EXT_MOTOR_RANGE 
        OCR4A = motor[2]<<3; //  pin 6, rear left/ link2
      #else
    ...
    #if (NUMBER_MOTOR > 3)
      #ifndef EXT_MOTOR_RANGE 
        OCR3B = motor[3]<<3; //  pin 2, front left/ link4
      #else
...
/**************************************************************************************/
void writeAllMotors(uint16_t mc) {   // Sends commands to all motors
...
void motorValueCheck()
{
  uint16_t maxMotor, maxThrottle;
  uint8_t i;
  ...
  for(i=1; i< NUMBER_MOTOR; i++)
    {
      ...
      if(throttle_pid_term[i] > maxThrottle) maxThrottle = throttle_pid_term[i];
    }
