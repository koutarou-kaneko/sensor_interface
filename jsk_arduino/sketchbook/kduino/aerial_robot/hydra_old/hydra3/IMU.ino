...
void computeIMU () {
  ...
  //gyro only: the delay to read 2 consecutive values can be reduced to only 0.65ms
    ...
    //bakui for gyro  transform cog
    gyro_cog.data[ROLL] = (int16_t)((int32_t)((int32_t)imu.gyroData[ROLL] * rotateAngle[0] + (int32_t)imu.gyroData[PITCH] * (- rotateAngle[1])) >> 10); //wired
    gyro_cog.data[PITCH] = (int16_t)((int32_t)((int32_t)imu.gyroData[ROLL] * rotateAngle[1] + (int32_t)imu.gyroData[PITCH] * (rotateAngle[0])) >> 10);
    gyro_cog.data[YAW] =  imu.gyroData[YAW];
}
...
// this one is also used for ALT HOLD calculation, should not be changed
  ...
  #define ACC_LPF_FACTOR 4 // 3 means a LPF of 32;  original is 4  that means a LPF of 128
  #define ACC_LPF_FACTOR_ANOTHER 5
#endif
...
static t_int32_t_vector EstG32Cog;
#if MAG
...
void getEstimatedAttitude(){
  ...
  for (axis = 0; axis < 3; axis++) {
    ...
    accLPF32Another[axis]    -= accLPF32Another[axis]>>ACC_LPF_FACTOR_ANOTHER;
    accLPF32Another[axis]    += imu.accADC[axis];
    imu.accSmoothAnother[axis]    = accLPF32Another[axis]>>ACC_LPF_FACTOR_ANOTHER;
  ...
  //for attitude transform cog
  EstG32Cog.V.X = (rotateAngle[0] * EstG32.V.X + (-rotateAngle[1]) * EstG32.V.Y) >> 10; // 1024 multiply
  EstG32Cog.V.Y = (rotateAngle[1] * EstG32.V.X + rotateAngle[0] * EstG32.V.Y) >> 10;
  EstG32Cog.V.Z = EstG32.V.Z;
  att_cog.angle[ROLL]  = _atan2(EstG32Cog.V.Y , EstG32Cog.V.Z);
  int32_t sqGY_sqGZ_Cog = sq(EstG32Cog.V.Y) + sq(EstG32Cog.V.Z);
  att_cog.angle[PITCH] = _atan2(-EstG32Cog.V.X , InvSqrt(sqGY_sqGZ_Cog)*sqGY_sqGZ_Cog);
