...
static struct {
  pid_  pid_gyro[PIDITEMS];
  pid_  pid_att[PIDITEMS];
} dynamic_gain;
...
//for hydra, parameter; bakui
static int16_t qMatrix[12] = {-1024, -1024, +1024, +1024, +1024, -1024, +1024, -1024 -1024, +1024, +1024, -1024}; //Q, QUADX(Output.ino 1044)
static uint16_t iPrincipalRate[3] = {1, 1, 1}; // Ixx, Iyy, Izz
static int16_t rotateAngle[2] = {1024, 0}; //cos_theta, sin_theta

static  int16_t PTermTemp[3] = {0,0,0};
static  int16_t ITermTemp[3] = {0,0,0};
static  int16_t DTermTemp[3] = {0,0,0};

static bool biasSetFlag = false;
static  int32_t motorBias[4] = {0,0,0,0};
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  for(axis=0;axis<3;axis++)
    {
      ...
      dynP8[axis] = (uint16_t)conf.pid[axis].P8;
      dynD8[axis] = (uint16_t)conf.pid[axis].D8;
    }
  rcCommand[THROTTLE] = constrain(rcData[THROTTLE], MINTHROTTLE,2000);
  ...
  for(axis=0;axis<3;axis++) {
    ...
    if(axis!=2) { //ROLL & PITCH
      ...
      rcCommand[axis] = lookupPitchRollRC[tmp2] + ((tmp-(tmp2<<7)) * (lookupPitchRollRC[tmp2+1]-lookupPitchRollRC[tmp2])>>7); //by bakui, about 450(45degree,  the calc errorAngle
    ...
    dynP8[axis] = (uint16_t)conf.pid[axis].P8*prop1>>7; // was /100, is /128 now(bakui kai)
    dynD8[axis] = (uint16_t)conf.pid[axis].D8*prop1>>7; // was /100, is /128 now(bakui kai)
...
#if 0 //add by bakui, for debug
  SERVO_1_PINMODE;
#endif

  //for dynamic gain setup
  dynamic_gain.pid_att[ROLL].P8 = conf.pid[PIDLEVEL].P8;
  dynamic_gain.pid_att[ROLL].I8 = conf.pid[PIDLEVEL].I8;
  dynamic_gain.pid_att[ROLL].D8 = conf.pid[PIDLEVEL].D8;
  dynamic_gain.pid_att[PITCH].P8 = conf.pid[PIDLEVEL].P8;
  dynamic_gain.pid_att[PITCH].I8 = conf.pid[PIDLEVEL].I8;
  dynamic_gain.pid_att[PITCH].D8 = conf.pid[PIDLEVEL].D8;

  dynamic_gain.pid_gyro[ROLL].P8 = conf.pid[ROLL].P8;
  dynamic_gain.pid_gyro[ROLL].D8 = conf.pid[ROLL].D8;
  dynamic_gain.pid_gyro[PITCH].P8 = conf.pid[PITCH].P8;
  dynamic_gain.pid_gyro[PITCH].D8 = conf.pid[PITCH].D8;
  dynamic_gain.pid_gyro[YAW].P8 = conf.pid[YAW].P8;
  dynamic_gain.pid_gyro[YAW].D8 = conf.pid[YAW].D8;
 ...
 //*********************************** 
  ...
  for(axis=0;axis<3;axis++) {
    if ((f.ANGLE_MODE || f.HORIZON_MODE) && axis<2 ) { // MODE relying on ACC
      ...
      errorAngle = constrain((rcCommand[axis]<<1),-500,+500) - att_cog.angle[axis] + conf.angleTrim[axis]; //16 bits is ok here

      PTermACC = ((int32_t)errorAngle*conf.pid[PIDLEVEL].P8)>>7;                          // 32 bits is needed for calculation: errorAngle*P8[PIDLEVEL] could exceed 32768   16 bits is ok for result, (bakui kai)
      PTermACC = constrain(PTermACC,-conf.pid[PIDLEVEL].D8*5,+dynamic_gain.pid_att[ROLL].D8*5); //(bakui kai)

      errorAngleI[axis]     = constrain(errorAngleI[axis]+errorAngle,-10000,+10000);    // WindUp     //16 bits is ok here
      ITermACC              = ((int32_t)errorAngleI[axis]*conf.pid[PIDLEVEL].I8)>>12;            // 32 bits is needed for calculation:10000*I8 could exceed 32768   16 bits is ok for result(bakui kai)
    }
    if ( !f.ANGLE_MODE || f.HORIZON_MODE || axis == 2 ) { // MODE relying on GYRO or YAW axis
      if (abs(rcCommand[axis])<500) error =       (rcCommand[axis]<<6)/conf.pid[axis].P8 ; // 16 bits is needed for calculation: 500*64 = 32000      16 bits is ok for result if P8>5 (P>0.5) (bakui kai)
      else error = ((int32_t)rcCommand[axis]<<6)/conf.pid[axis].P8 ; // 32 bits is needed for calculation

      error -= gyro_cog.data[axis];

      PTermGYRO = rcCommand[axis];
      

      //bakui, strange, should remove this part => the reason of yaw drift
      errorGyroI[axis]  = constrain(errorGyroI[axis]+error,-16000,+16000);         // WindUp   16 bits is ok here
      //next step
      //errorGyroI[axis]  = constrain(errorGyroI[axis]+error,-24000,+24000);         // WindUp   16 bits is ok here

      if (abs(gyro_cog.data[axis])>640) errorGyroI[axis] = 0;
    ...
    PTerm -= ((int32_t)gyro_cog.data[axis]*dynP8[axis])>>6; // 32 bits is needed for calculation   

#if 0 //pidd mode
    delta          = gyro_cog.data[axis] - lastGyro[axis];  // 16 bits is ok here, the dif between 2 consecutive gyro reads is limited to 800
    lastGyro[axis] = gyro_cog.data[axis];
    ...
    //bakui debug, not good
     PTermTemp[axis] = PTerm;
      ITermTemp[axis] = ITerm;
      DTermTemp[axis] = -DTerm;
  }

  mixTable();
  writeServos();
  writeMotors();
  //digitalWrite(44, HIGH-digitalRead(44));   // blink the led
}
