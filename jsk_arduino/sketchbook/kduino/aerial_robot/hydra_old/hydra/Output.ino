...
void writeServos() {
      ...
      for(uint8_t i = (PRI_SERVO_FROM-1); i < PRI_SERVO_TO; i++){
        #if defined(PROI) || (defined(PROMICRO) && defined(HWPWM6)) || (defined(MEGA) && defined(MEGA_HW_PWM_SERVOS))
...
#else //bakui edited
      motor[0] = (int32_t)rcCommand[THROTTLE] + (int32_t)(((int32_t)axisPID[ROLL]* qMatrix[0] + (int32_t)axisPID[PITCH]*qMatrix[4] + YAW_DIRECTION * (int32_t)axisPID[YAW]*qMatrix[8])>>10);
      motor[1] = (int32_t)rcCommand[THROTTLE] + (int32_t)(((int32_t)axisPID[ROLL]* qMatrix[1] + (int32_t)axisPID[PITCH]*qMatrix[5] + YAW_DIRECTION * (int32_t)axisPID[YAW]*qMatrix[9])>>10);
      motor[2] = (int32_t)rcCommand[THROTTLE] + (int32_t)(((int32_t)axisPID[ROLL]* qMatrix[2] + (int32_t)axisPID[PITCH]*qMatrix[6] + YAW_DIRECTION * (int32_t)axisPID[YAW]*qMatrix[10])>>10);
      motor[3] = (int32_t)rcCommand[THROTTLE] + (int32_t)(((int32_t)axisPID[ROLL]* qMatrix[3] + (int32_t)axisPID[PITCH]*qMatrix[7] + YAW_DIRECTION * (int32_t)axisPID[YAW]*qMatrix[11])>>10);
#endif
    #endif
    #ifdef Y4
