...
// convert to right-handed system and calc and reverse
//  right-handed system
//    +---LED---+      ^ X
//    |         |      |
//    |         USB  Z x -> Y
//    |         |
//    +---------+
#ifndef AXIS_ORIENTATION
#define AXIS_ORIENTATION(v)
#else //AXIS_ORIENTATION
#define AXIS_ORIENTATION_Xp90(v) {int16_t tmp=v[2];v[2]=v[1];v[1]=-tmp;}  // rot  90deg at X axis
#define AXIS_ORIENTATION_Xm90(v) {int16_t tmp=v[1];v[1]=v[2];v[2]=-tmp;}  // rot -90deg at X axis
#define AXIS_ORIENTATION_Yp90(v) {int16_t tmp=v[0];v[0]=v[2];v[2]=-tmp;}  // rot  90deg at Y axis
#define AXIS_ORIENTATION_Ym90(v) {int16_t tmp=v[2];v[2]=v[0];v[0]=-tmp;}  // rot -90deg at Y axis
#define AXIS_ORIENTATION_Zp90(v) {int16_t tmp=v[1];v[1]=v[0];v[0]=-tmp;}  // rot  90deg at Z axis
#define AXIS_ORIENTATION_Zm90(v) {int16_t tmp=v[0];v[0]=v[1];v[1]=-tmp;}  // rot -90deg at Z axis
#define AXIS_ORIENTATION_X180(v) {v[1]=-v[1];v[2]=-v[2];}  // rot 180deg at X axis
#define AXIS_ORIENTATION_Y180(v) {v[2]=-v[2];v[0]=-v[0];}  // rot 180deg at Y axis
#define AXIS_ORIENTATION_Z180(v) {v[0]=-v[0];v[1]=-v[1];}  // rot 180deg at Z axis
#define AXIS_ORIENTATION_ROT(v,rot) AxisOrientation(v,rot)
void AxisOrientation(int16_t v[], int16_t rot[]) {
  int16_t ans[3];
  ans[0] = rot[0]*v[0]+rot[1]*v[1]+rot[2]*v[2];
  ans[1] = rot[3]*v[0]+rot[4]*v[1]+rot[5]*v[2];
  ans[2] = rot[6]*v[0]+rot[7]*v[1]+rot[8]*v[2];
  v[0] = ans[0];
  v[1] = ans[1];
  v[2] = ans[2];
}
#endif //AXIS_ORIENTATION
...
void GYRO_Common() {
  ...
  if (calibratingG>0) {
    for (axis = 0; axis < 3; axis++) {
      ...
      gyroZero[axis]=0;
      if (calibratingG == 1) {
        gyroZero[axis]=(g[axis]+256)>>9;  // round
...
void Device_Mag_getADC() {
  ...
  AXIS_CONV_MAG_M2R(imu.magADC);
  AXIS_ORIENTATION(imu.magADC);
  AXIS_CONV_MAG_R2M(imu.magADC);
}
// ************************************************************************************************************
...
void Gyro_getADC () {
  ...
  AXIS_CONV_GYRO_M2R(imu.gyroADC);
  AXIS_ORIENTATION(imu.gyroADC);
  AXIS_CONV_GYRO_R2M(imu.gyroADC);
  GYRO_Common();
}
void ACC_init () {
...
void ACC_getADC () {
  ...
  AXIS_CONV_ACC_M2R(imu.accADC);
  AXIS_ORIENTATION(imu.accADC);
  AXIS_CONV_ACC_R2M(imu.accADC);
  ACC_Common();
}
